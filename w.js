      ...
      if (info.param.eventKey == '绑定设备') {
        ...
        var reply = {
               ...
               description: '问: 定位器有没有年费？'+ '\n' +
                            ...
                            '问: 如何撤销报警? '+ '\n' +
                            '答: 请通过微信发送待机指令，格式为：设备名称@待机 比如：1141023233@待机 设备会每30分钟报告一次精确位置信息'+ '\n' +
                            '问: 电池电量低时，如何处理？'+ '\n' +
        ...
        var reply = {
               ...
               description: '绑定指令：【帐号#用户#密码】'+ '\n' +
                            ...
                            '报警指令：【设备号@报警】 '+ '\n' +'报警指令执行时定位器每1小时返回一次定位数据，直到电源20%时自动进入待机指令'+ '\n' +
                            '待机指令：【设备号@待机】'+ '\n' +'待机指令执行时，定位器早上、中午、晚上、凌晨各返回一次定位数据'+ '\n' +
                            '睡眠指令：【设备号@睡眠】'+ '\n' +'待机指令执行时，定位器每天12点返回一次定位数据'+ '\n' +
                            '改名指令：【设备号@改名@新的设备名称】'+ '\n' +'系统返改名成功'+ '\n' +
                            '如：'+ '\n' +
    ...
    var s2 = split[2];
                   mysql.query('SELECT accountID,userID,notes FROM User WHERE notes = "'+info.uid+'"', function(err, fields) {
                    ...
                    if (s1 == "报警"){
                       ...
                       res_string = "报警指令已提交,定位器下次签到时，会每小时回传一次GPS位置，追踪完毕后，请及时提交待机指令，否则电池很快耗尽"
                       }
                    else if ( s1 == "待机"){
                        ...
                        res_string =  "待机指令已提交，定位器每天签到服务器4次"
                       }
                    else if ( s1 == "睡眠"){
                       ...
                       res_string = "睡眠指令已提交,定位器每天中午12点签到服务器一次，建议提交待机指令，让定位器每天签到服务器4次"
                       }
                     else if (s1 == "紧急"){
                          ...
                          res_string = "紧急指令已提交,定位器下次签到时，会马上启动GPS定位！追踪完毕后，请及时提交待机指令，否则电池会在1天内耗尽"                          
                       }
                     else if (s1 == "改名"){
