...
class Supreme {
	...
	function i18n() {
		...
		$supreme->texttextdomain_loaded['supreme-core'] = supreme_load_framework_textdomain( 'supreme-core' );
	...
	function theme_support() {
		...
		if ( defined( 'WPSEO_VERSION' ) )
			remove_theme_support( 'supreme-core-seo' );
	...
	function functions() {
		...
		require_once( trailingslashit( SUPREME_FUNC ) . 'meta.php' );
		...
		require_if_theme_supports( 'supreme-core-seo', trailingslashit( SUPREME_FUNC ) . 'core-seo.php' );
		...
		require_if_theme_supports( 'supreme-core-sidebars', trailingslashit( SUPREME_FUNC ) . 'sidebars.php' );

		/* Load the widgets if supported. */

		require_if_theme_supports( 'supreme-core-widgets', trailingslashit( SUPREME_FUNC ) . 'widgets_functions.php' );
		require_if_theme_supports( 'supreme-core-widgets', trailingslashit( SUPREME_FUNC ) . 'widgets.php' );
		require_if_theme_supports( 'supreme-core-widgets', trailingslashit( SUPREME_CLASSES ) . 'twitter.php' );
		
		require_if_theme_supports( 'supreme-core-widgets', trailingslashit( SUPREME_CLASSES ) . 'facebook.php' );

		/* Load the template hierarchy if supported. */
		require_if_theme_supports( 'supreme-core-template-hierarchy', trailingslashit( SUPREME_FUNC ) . 'template-hierarchy.php' );
		if(file_exists(get_template_directory().'/library/framework_update.php')){
			require_once(get_template_directory().'/library/framework_update.php'); // file for auto updating feature of framework
		}
	}
	...
	function extensions() {
		...
		require_if_theme_supports( 'get-the-image', trailingslashit( SUPREME_EXTENSIONS ) . 'get-the-image.php' );
		...
		require_if_theme_supports( 'post-stylesheets', trailingslashit( SUPREME_EXTENSIONS ) . 'post-stylesheets.php' );
		/* File for mobile device compatibility */
		if(file_exists(get_template_directory().'/library/classes/mobile-detect.php')){
		//	require_once(get_template_directory().'/library/classes/mobile-detect.php');
			//global $detect;
