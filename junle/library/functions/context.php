 ...
 * @global $supreme The global Hybrid object.
 * @return array $supreme->context Several contexts based on the current page.
 */
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_date () ) {
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$doctitle = sprintf( __( 'Archive for %1$s', 'supreme-core' ), get_the_time( __( 'g:i a', 'supreme-core' ) ) );

			elseif ( get_query_var( 'minute' ) )
				$doctitle = sprintf( __( 'Archive for minute %1$s', 'supreme-core' ), get_the_time( __( 'i', 'supreme-core' ) ) );

			elseif ( get_query_var( 'hour' ) )
				$doctitle = sprintf( __( 'Archive for %1$s', 'supreme-core' ), get_the_time( __( 'g a', 'supreme-core' ) ) );

			elseif ( is_day() )
				$doctitle = sprintf( __( 'Archive for %1$s', 'supreme-core' ), get_the_time( __( 'F jS, Y', 'supreme-core' ) ) );

			elseif ( get_query_var( 'w' ) )
				$doctitle = sprintf( __( 'Archive for week %1$s of %2$s', 'supreme-core' ), get_the_time( __( 'W', 'supreme-core' ) ), get_the_time( __( 'Y', 'supreme-core' ) ) );

			elseif ( is_month() )
				$doctitle = sprintf( __( 'Archive for %1$s', 'supreme-core' ), single_month_title( ' ', false) );

			elseif ( is_year() )
				$doctitle = sprintf( __( 'Archive for %1$s', 'supreme-core' ), get_the_time( __( 'Y', 'supreme-core' ) ) );
		}

		/* For any other archives. */
		else {
			$doctitle = __( 'Archives', 'supreme-core' );
		}
	}

	/* If viewing a search results page. */
	elseif ( is_search() )
		$doctitle = sprintf( __( 'Search results for &quot;%1$s&quot;', 'supreme-core' ), esc_attr( get_search_query() ) );

	/* If viewing a 404 not found page. */
	elseif ( is_404() )
		$doctitle = __( '404 Not Found', 'supreme-core' );

	/* If the current page is a paged page. */
	if ( ( ( $page = $wp_query->get( 'paged' ) ) || ( $page = $wp_query->get( 'page' ) ) ) && $page > 1 )
		$doctitle = sprintf( __( '%1$s Page %2$s', 'supreme-core' ), $doctitle . $separator, number_format_i18n( $page ) );
