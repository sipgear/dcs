...
add_action( 'wp_head', 'supreme_meta_robots', 1 );
add_action( 'wp_head', 'supreme_meta_author', 1 );
add_action( 'wp_head', 'supreme_meta_copyright', 1 );
add_action( 'wp_head', 'supreme_meta_revised', 1 );
add_action( 'wp_head', 'supreme_meta_description', 1 );
add_action( 'wp_head', 'supreme_meta_keywords', 1 );
 ...
 * @return void
 */
function supreme_meta_copyright() {

	/* If viewing a singular post, get the post month and year. */
	if ( is_singular() )
		$date = get_the_time( esc_attr__( 'F Y', 'supreme-core' ) );

	/* For all other views, get the current year. */
	else
		$date = date( esc_attr__( 'Y', 'supreme-core' ) );

	/* Create the HTML for the copyright meta tag. */
	$copyright = '<meta name="copyright" content="' . sprintf( esc_attr__( 'Copyright (c) %1$s', 'supreme-core' ), $date ) . '" />' . "\n";
...
function supreme_meta_revised() {
	...
	if ( is_singular() )
		$revised = '<meta name="revised" content="' . get_the_modified_time( esc_attr__( 'l, F jS, Y, g:i a', 'supreme-core' ) ) . '" />' . "\n";
 ...
 * @return void
 */
function supreme_meta_description() {
 ...
 * @return void
 */
function supreme_meta_keywords() {
