...
function supreme_wp_link_shortcode() {
	return '<a class="wp-link" href="http://wordpress.org" title="' . esc_attr__( 'State-of-the-art semantic personal publishing platform', 'supreme-core' ) . '"><span>' . __( 'WordPress', 'supreme-core' ) . '</span></a>';
}
...
function supreme_theme_link_shortcode() {
	...
	return '<a class="theme-link" href="' . esc_url( $theme->get( 'ThemeURI' ) ) . '" title="' . sprintf( esc_attr__( '%s WordPress Theme', 'supreme-core' ), $theme->get( 'Name' ) ) . '"><span>' . esc_attr( $theme->get( 'Name' ) ) . '</span></a>';
}
...
function supreme_loginout_link_shortcode() {
	if ( is_user_logged_in() )
		$out = '<a class="logout-link" href="' . esc_url( wp_logout_url( site_url( $_SERVER['REQUEST_URI'] ) ) ) . '" title="' . esc_attr__( 'Log out', 'supreme-core' ) . '">' . __( 'Log out', 'supreme-core' ) . '</a>';
	else
		$out = '<a class="login-link" href="' . esc_url( wp_login_url( site_url( $_SERVER['REQUEST_URI'] ) ) ) . '" title="' . esc_attr__( 'Log in', 'supreme-core' ) . '">' . __( 'Log in', 'supreme-core' ) . '</a>';
...
function supreme_query_counter_shortcode() {
	if ( current_user_can( 'edit_theme_options' ) )
		return sprintf( __( 'This page loaded in %1$s seconds with %2$s database queries.', 'supreme-core' ), timer_stop( 0, 3 ), get_num_queries() );
	return '';
}
...
function supreme_comment_published_shortcode() {
	$link = '<span class="published">' . sprintf( __( '%1$s at %2$s', 'supreme-core' ), '<abbr class="comment-date" title="' . get_comment_date( esc_attr__( 'l, F jS, Y, g:i a', 'supreme-core' ) ) . '">' . get_comment_date() . '</abbr>', '<abbr class="comment-time" title="' . get_comment_date( esc_attr__( 'l, F jS, Y, g:i a', 'supreme-core' ) ) . '">' . get_comment_time() . '</abbr>' ) . '</span>';
	return $link;
}
...
function supreme_comment_permalink_shortcode( $attr ) {
	...
	$link = '<a class="permalink" href="' . esc_url( get_comment_link( $comment->comment_ID ) ) . '" title="' . sprintf( esc_attr__( 'Permalink to comment %1$s', 'supreme-core' ), $comment->comment_ID ) . '">' . __( 'Permalink', 'supreme-core' ) . '</a>';
...
function supreme_comment_edit_link_shortcode( $attr ) {
	...
	$link = '<a class="comment-edit-link" href="' . esc_url( $edit_link ) . '" title="' . sprintf( esc_attr__( 'Edit %1$s', 'supreme-core' ), $comment->comment_type ) . '"><span class="edit">' . __( 'Edit', 'supreme-core' ) . '</span></a>';
...
function supreme_comment_reply_link_shortcode( $attr ) {
	...
	$defaults = array(
		'reply_text' => __( 'Reply', 'supreme-core' ),
		'login_text' => __( 'Log in to reply.', 'supreme-core' ),
